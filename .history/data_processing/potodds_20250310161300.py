...
def get_call_amount_postflop(postflop_action, hero_pos):
    ...
    # If "dealcards" exists, isolate only the latest betting round
    if "dealcards/" in postflop_action:
        tokens = postflop_action.split("/")
        last_deal_idx = max(i for i, token in enumerate(tokens) if token.startswith("dealcards/"))
        relevant_actions = tokens[last_deal_idx + 1 :]  # Only consider actions after last "dealcards/"
    else:
        relevant_actions = postflop_action.split("/")  # No "dealcards", consider everything
